...
export default function RoomSelect({setRoomBranch, currentRoomBranch , roomsBranch, lable} : { currentRoomBranch : string , roomsBranch : string[] | undefined, setRoomBranch : Dispatch<SetStateAction<string>> , lable : string}) {
    ...
    <div>
      <label className="block mb-2 text-neutral-950 font-bold">{lable}</label>
          ...
          <div className="w-[200px] relative bg-transparent border-2 border-zinc-400 rounded cursor-pointer px-3 py-2 flex items-center justify-between" onClick={() => setShow(!show)}>
              <p>{ roomsBranch?.find(item => item === currentRoomBranch) ? currentRoomBranch : 'Chon Phong'  }</p>
