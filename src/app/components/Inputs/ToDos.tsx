"use client";
import { Dispatch, SetStateAction, useRef, ChangeEvent, useEffect } from "react";
...
export default function ToDos({
  ...
  comportArray: string[];
  setComportArray: Dispatch<SetStateAction<string[]>>;
  currentComport: string;
  setCurrentComport: Dispatch<SetStateAction<string>>;
  initialComports? : string[]
}) {
  useEffect(() => {
    if (initialComports) {
      setComportArray(initialComports);
    }
  }, [initialComports]);
    ...
    <div className="p-2">
      <div className="flex items-start justify-start space-x-10 mb-7">
        <input
          ...
          className={`w-[300px] p-2 bg-transparent outline-none border-2 border-zinc-400 focus:border-indigo-500 active:border-indigo-500 rounded-[7px] placeholder:text-neutral-600 text-neutral-900 `}
        />
        <button
          onClick={handleClick}
      ...
      <ul className="flex items-start justify-start flex-wrap">
        {comportArray?.map((currentItem, id) => (
          <li
            className="no-list-style px-3 mr-3 py-1 rounded-sm bg-orange-500 text-white my-1"
            key={id}
          >
